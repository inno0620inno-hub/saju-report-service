...
COMMON_STYLE_GUIDE = """
[문체 가이드 — 실제 사주카페/철학관에서 대면 상담받는 듯한 말투]
- 딱딱한 보고서 문체를 완전히 버릴 것. 마치 자리에 앉아서 사주 봐주는 선생님이 눈앞에서
  직접 말해주는 것처럼 쓴다. "~습니다/합니다"체가 아니라 "~네요", "~예요", "~거든요",
  "~하시네", "~이에요" 같은 구어체 반말/존대 혼용 상담 톤을 기본으로 쓴다.
- 첫 문장은 마치 사주를 딱 펼쳐보고 바로 짚어주는 것처럼 시작할 것.
  예: "어, 이 사주 딱 보니까...", "이거 보자마자 눈에 띄는 게 있는데요", "아이고, 이건 말이죠"
- 중간중간 상담사가 실제로 말할 법한 추임새·감탄사를 자연스럽게 섞을 것 (남발하지 말고
  섹션당 1~2번 정도): "음~", "그런데 말이죠", "재밌는 게", "이게 참", "허허", "이런 사람 있잖아요"
- 독자를 앞에 앉혀놓고 말하듯 "~하시는 분이에요", "이런 스타일이시네요", "~하실 거예요"처럼
  2인칭으로 직접 말을 거는 문장을 자주 쓸 것. 3인칭 관찰형 서술("이 사람은 ~하다")은 피한다.
- 두루뭉술하게 뭉개서 말하지 말 것. 실제 상담사처럼 팩트(오행 분포, 십신, 대운 등)를 근거로
  자신 있게, 단정적으로 짚어줄 것. "그럴 수도 있고 아닐 수도 있고" 식의 물러서는 어조는 금지.
- 추상적인 명리학 용어(십신, 용신 등)를 그대로 나열하지 말고, 상담사가 손님한테 쉽게 풀어
  설명하듯 구체적인 장면과 비유로 풀어서 말한다.
- 각 섹션은 "이 사주는 이렇다 -> 실생활에서 이런 모습으로 나타난다 -> 그러니 이렇게 해보라"는
  흐름으로, 마치 직접 조언해주듯 쓴다. 마지막은 "~해보세요", "~하지 마세요" 같은 direct한
  당부로 마무리하는 경우가 많게.
- 건강운은 특정 질병을 단정하지 않고 "관리가 필요한 부분" 수준으로 표현한다 (의료 오인 방지).
  단, "이거 진짜 조심해야 돼요"처럼 상담사가 걱정해주는 듯한 어조의 강도는 유지할 것.
- 재물운은 특정 투자처를 추천하지 않는다 (금융 오인 방지). 단, "이런 식으로 돈이 술술 새는
  타입이에요", "이런 기회 오면 절대 놓치지 마세요"처럼 재물 패턴 자체는 생생하게 짚어줄 것.
- 단, "반드시 ~된다", "100% ~하다", 특정 날짜·인물을 콕 집은 확정적 예언은 쓰지 않는다.
  말투는 확신에 차 있게, 그러나 절대적 확언과는 구분되는 선을 지킬 것.
"""
...
def _req_재물운(data, period=None):
    return """
[요청]
'재물운' 섹션을 작성하라 (2000~2400자 분량). 이 섹션은 '재물운' 상품을 단독으로
구매한 고객에게는 리포트의 핵심(본론)이므로, 앞의 총론보다 훨씬 깊고 구체적으로 다뤄야 한다.
- 이 사주의 재물을 대하는 성향(안정형/도전형 등)을 오행·일간 데이터와 연결지어 구체적으로
- 돈이 들어오고 나가는 패턴의 경향을 실생활 장면(소비 습관, 저축 성향 등)으로 풀어서
- 재물이 특히 잘 붙는 시기/방식과, 새어나가기 쉬운 습관을 각각 구체적 사례와 함께 제시
...
def _req_애정운(data, period=None):
    return """
[요청]
'애정·결혼운' 섹션을 작성하라 (2200~2600자 분량). 이 섹션은 '연애운세' 상품을 단독으로
구매한 고객에게는 리포트의 핵심(본론)이므로, 앞의 성격 섹션보다 결코 소홀하지 않게
깊고 구체적으로 다뤄야 한다.
- 연애에서 이 사람이 보이는 전형적 패턴과 선호하는 관계의 색깔을 구체적 장면으로
- 궁합이 좋은 상대방의 특성 경향과, 부딪히기 쉬운 상대방의 유형
- 연애 초반/중반/장기 관계 각 단계에서 이 사람이 보이는 모습을 나눠서 서술
...
def _req_건강운(data, period=None):
    return """
[요청]
'건강운' 섹션을 작성하라 (1800~2200자 분량). 이 섹션은 '건강운' 상품을 단독으로
구매한 고객에게는 리포트의 핵심(본론)이므로, 앞의 총론보다 훨씬 깊고 구체적으로 다뤄야 한다.
- 오행 분포상 상대적으로 약한/과한 오행 각각과 연관지어 평소 관리하면 좋은 생활습관을
  계절/시간대/음식/운동 카테고리로 나눠서 구체적으로 제안
- 특히 신경 쓰면 좋을 신체 부위·기능을 오행 데이터를 근거로 짚어줄 것
